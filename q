...
[1mDESCRIPTION[0m
       These functions return information about a file, in the buffer pointed to by [4mstatbuf[24m.  No permissions are required on the file itself, but—in the case of [1mstat[22m(), [1mfstatat[22m(), and [1mlstat[22m()—exe‐
       cute (search) permission is required on all of the directories in [4mpathname[24m that lead to the file.
   ...
   [1mThe stat structure[0m
       ...
       [4mNote[24m: for performance and simplicity reasons, different fields in the [4mstat[24m structure may contain state information from different moments during the execution of the system call.  For exam‐
       ple,  if  [4mst_mode[24m  or [4mst_uid[24m is changed by another process by calling [1mchmod[22m(2) or [1mchown[22m(2), [1mstat[22m() might return the old [4mst_mode[24m together with the new [4mst_uid[24m, or the old [4mst_uid[24m together with
       the new [4mst_mode[24m.
   ...
   [1mfstatat()[0m
       ...
       If the pathname given in [4mpathname[24m is relative, then it is interpreted relative to the directory referred to by the file descriptor [4mdirfd[24m (rather than relative to the current working  direc‐
       tory of the calling process, as is done by [1mstat[22m() and [1mlstat[22m() for a relative pathname).
       ...
       [1mAT_EMPTY_PATH [22m(since Linux 2.6.39)
              If  [4mpathname[24m  is an empty string, operate on the file referred to by [4mdirfd[24m (which may have been obtained using the [1mopen[22m(2) [1mO_PATH [22mflag).  In this case, [4mdirfd[24m can refer to any type of
              file, not just a directory, and the behavior of [1mfstatat[22m() is similar to that of [1mfstat[22m().  If [4mdirfd[24m is [1mAT_FDCWD[22m, the call operates on the current  working  directory.   This  flag  is
              Linux-specific; define [1m_GNU_SOURCE [22mto obtain its definition.
       ...
       [1mAT_SYMLINK_NOFOLLOW[0m
              If  [4mpathname[24m  is  a  symbolic  link,  do not dereference it: instead return information about the link itself, like [1mlstat[22m().  (By default, [1mfstatat[22m() dereferences symbolic links, like
              [1mstat[22m().)
...
[1mERRORS[0m
       ...
       [1mEOVERFLOW[0m
              [4mpathname[24m or [4mfd[24m refers to a file whose size, inode number, or number of blocks cannot be represented in, respectively, the types [4moff_t[24m, [4mino_t[24m, or [4mblkcnt_t[24m.  This error can occur when,
              for example, an application compiled on a 32-bit platform without [4m-D_FILE_OFFSET_BITS=64[24m calls [1mstat[22m() on a file whose size exceeds [4m(1<<31)-1[24m bytes.
...
[1mHISTORY[0m
       ...
       According to POSIX.1-2001, [1mlstat[22m() on a symbolic link need return valid information only in the [4mst_size[24m field and the file type of the [4mst_mode[24m field of  the  [4mstat[24m  structure.   POSIX.1-2008
       tightens the specification, requiring [1mlstat[22m() to return valid information in all fields except the mode bits in [4mst_mode[24m.

       Use  of the [4mst_blocks[24m and [4mst_blksize[24m fields may be less portable.  (They were introduced in BSD.  The interpretation differs between systems, and possibly on a single system when NFS mounts
       are involved.)

   [1mC library/kernel differences[0m
       Over time, increases in the size of the [4mstat[24m structure have led to three successive versions of [1mstat[22m(): [4msys_stat[24m() (slot [4m__NR_oldstat[24m),  [4msys_newstat[24m()  (slot  [4m__NR_stat[24m),  and  [4msys_stat64()[0m
       (slot  [4m__NR_stat64[24m)  on 32-bit platforms such as i386.  The first two versions were already present in Linux 1.0 (albeit with different names); the last was added in Linux 2.4.  Similar re‐
       marks apply for [1mfstat[22m() and [1mlstat[22m().
       ...
       [4mstat64[24m Even larger [4mst_ino[24m field, larger [4mst_uid[24m and [4mst_gid[24m fields to accommodate the Linux-2.4 expansion of UIDs and GIDs to 32 bits, and various other enlarged fields and further padding in
              the structure.  (Various padding bytes were eventually consumed in Linux 2.6, with the advent of 32-bit device IDs and nanosecond components for the timestamp fields.)

       The glibc [1mstat[22m() wrapper function hides these details from applications, invoking the most recent version of the system call provided by the kernel, and repacking the  returned  information
...
Linux man-pages 6.7                                                                          2023-10-31                                                                                      [4mstat[24m(2)
